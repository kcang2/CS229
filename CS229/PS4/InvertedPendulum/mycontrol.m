...
trans_count=zeros(NUM_STATES,NUM_STATES,2);
trans_prob=ones(NUM_STATES,NUM_STATES,2)./NUM_STATES;
reward_count=zeros(NUM_STATES,2);
...
% here, and say the overall algorithm has converged. 
NUM_CON_NO_LEARN_TRIALS=0;
while ((num_failures < max_failures)&&...
        (NUM_CON_NO_LEARN_TRIALS < NO_LEARNING_THRESHOLD))
      ...
      score1 = trans_prob(state,:,1)*value;
      score2 = trans_prob(state,:,2)*value;
      if (score1>score2)
        action=1;
      ...
      trans_count(state,new_state,action) = trans_count(state,new_state,action) + 1;
      reward_count(new_state,1) = reward_count(new_state,1) + R;
      reward_count(new_state,2) = reward_count(new_state,2) + 1;
      ...
      if (new_state==NUM_STATES)
        ...
        % component (and thus keep it at the initialized uniform distribution).
          ...
          nani = find(~isfinite(trans_prob));
          trans_prob(nani) = 1/NUM_STATES;
          rewarded = find(reward_count(:,2)~=0);
          reward(rewarded) = reward_count(rewarded, 1)./reward_count(rewarded, 2);                
        ...
        oldvalue = zeros(size(value));
        value_iter = 0;
        
        while (max(abs(oldvalue-value))>TOLERANCE)
            value_iter = value_iter + 1;
            oldvalue = value;
            value = reward + max(GAMMA.*[trans_prob(:,:,1)*value trans_prob(:,:,2)*value] , [], 2);
        end
        
        if(value_iter==1)
            NUM_CON_NO_LEARN_TRIALS = NUM_CON_NO_LEARN_TRIALS+1;
        else
            NUM_CON_NO_LEARN_TRIALS = 0;
        end    
