...
for m = 1:rowscols(1) % as many rows as number of documents
  ...
  nums = sscanf(line, '%d');
  category(m) = nums(1);
  % nums(3:2:end-1) are the frequencies
  % nums(2:2:end-1) are the token #s, from the earliest token # to the
  % next, via step size. eg. 362,4,...
  % cumsum(nums(2:2:end-1)) would be the actualy token #s eg. 362,362+4,...
  % token #s and frequencies alternate, hence :2:
